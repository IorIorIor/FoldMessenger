 ...
 * it took, so "some phones are a minute late" is a number on a screen instead of
 * an impression from the room.
 *
 * Every duration is measured on this phone between two points in the same
 * method, so it carries no clock skew: six handsets that have never agreed on
 * the time still produce comparable figures.
 */
object AckSender {
    ...
    fun send(ctx: Context, phase: String, ms: Long, note: String = "") {
        val body = JSONObject().apply {
            put("phone", MessageStore.getPhoneId(ctx))
            put("phase", phase)
            put("ms", ms)
        ...
        executor.execute {
            try {
                val request = Request.Builder()
                    .url("${Config.NTFY_SERVER}/${Config.acksTopic()}")
